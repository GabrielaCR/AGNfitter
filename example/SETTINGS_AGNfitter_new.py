...
def CATALOG_settings():
   ...
   ##FREQUENCIES/WAVELENGTHS 
    ...
    cat['freq/wl_list'] = np.arange(2,147,3).tolist()  #138 147                            
    ...
    cat['flux_list'] = np.arange(3,148,3).tolist()      #139 148
                                        ## If ASCII: List of column indexes (int)
    #cat['flux_suffix'] = '_f'          ## If FITS: Common ending of all flux column names (str)    
    cat['fluxerr_list'] = np.arange(4,149,3).tolist() #140 149
                                        ## If ASCII: List of column indexes (int)
    ...
    cat['output_folder'] =  cat['workingpath'] +'SN12/' #if no special OUTPUT folder, leave default
